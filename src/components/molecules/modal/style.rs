...
impl Class for ModalTriggerProps {
    fn base(&self) -> &'static str {
        "px-4 py-2 text-sm font-medium select-none text-foreground bg-background border border-input rounded-global-radius shadow-sm shadow-global-shadow whitespace-nowrap cursor-pointer hover:bg-accent hover:text-accent-foreground"
    }
}
